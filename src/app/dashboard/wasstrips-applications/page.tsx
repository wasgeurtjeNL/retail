    ...
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col">
        ...
        <main className="flex-grow flex items-center justify-center p-4">
          <div className="max-w-2xl mx-auto bg-red-50 border-l-4 border-red-500 p-6 rounded">
      ...
      <main className="flex-grow">
        <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <header className="mb-8">
            <h1 className="text-3xl font-bold text-gray-900">Wasstrips Aanvragen Beheren</h1>
            <p className="mt-2 text-gray-600">Overzicht van alle wasstrips aanvragen en hun status.</p>
          </header>
          ...
          <div className="bg-white shadow rounded-lg px-6 py-4 mb-6">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between">
              <div className="mb-4 md:mb-0">
                 <h2 className="text-lg font-medium text-gray-900">Filter & Beheer</h2>
                  <p className="mt-1 text-sm text-gray-500">
                    Gebruik de onderstaande opties om de aanvragen te filteren en beheren.
                  </p>
              </div>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Betaling</th>
                      <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Retailer Status</th>
                      ...
                      <tr key={app.id} className="hover:bg-gray-50">
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="group relative flex items-center space-x-2">
                            {/* Zichtbare info */}
                            <div>
                              <div className="text-sm font-medium text-gray-900">{app.businessName || app.companyName || 'N/A'}</div>
                              <div className="text-sm text-gray-500">{app.contactName || 'N/A'}</div>
                            </div>
                            <FaEye className="w-4 h-4 text-gray-400 group-hover:text-gray-600 transition-colors"/>
                            
                            {/* Popover die op hover verschijnt */}
                            <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 w-52 z-10
                                            hidden group-hover:block 
                                            bg-white border border-gray-200 rounded-lg shadow-2xl
                                            p-3 transition-all duration-200">
                              <div className="font-bold text-sm text-gray-900 mb-1 border-b pb-1">{app.email}</div>
                              <div className="text-sm text-gray-700">{app.phone}</div>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                            <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">
                              {app.status === 'approved' ? 'Goedgekeurd' : app.status === 'order_ready' ? 'Bestelling binnen' : 'In afwachting'}
                            </span>
                          </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex flex-col space-y-1">
                            ...
                            <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                              app.deposit_status === 'paid' ? 'bg-green-100 text-green-800' :
                              app.deposit_status === 'sent' ? 'bg-yellow-100 text-yellow-800' :
                              app.deposit_status === 'failed' ? 'bg-red-100 text-red-800' :
                              'bg-gray-100 text-gray-800'
                            }`}>
                              {app.deposit_status === 'paid' ? '✅ €30 betaald' :
                               app.deposit_status === 'sent' ? '📧 Link verstuurd' :
                               app.deposit_status === 'failed' ? '❌ Mislukt' :
                               '⏳ Nog niet verstuurd'}
                            </span>
                            ...
                            <span className={`inline-flex items-center px-3 py-1.5 rounded-full text-xs font-medium border transition-all duration-300 hover:scale-105 shadow-sm ${
                              app.status === 'shipped' ? 'bg-gradient-to-r from-emerald-100 to-emerald-200 text-emerald-800 border-emerald-300' :
                              app.status === 'payment_selected' ? 'bg-gradient-to-r from-blue-100 to-blue-200 text-blue-800 border-blue-300 animate-pulse' :
                              app.status === 'order_ready' ? 'bg-gradient-to-r from-purple-100 to-purple-200 text-purple-800 border-purple-300' :
                              app.status === 'approved' ? 'bg-gradient-to-r from-green-100 to-green-200 text-green-800 border-green-300' :
                              app.status === 'pending' ? 'bg-gradient-to-r from-amber-100 to-amber-200 text-amber-800 border-amber-300 ring-2 ring-amber-200' :
                              'bg-gradient-to-r from-red-100 to-red-200 text-red-800 border-red-300'
                            }`}>
                              <span className="mr-1.5 text-sm">
                                {app.status === 'shipped' ? '🚚' :
                                 app.status === 'payment_selected' ? '💳' :
                                 app.status === 'order_ready' ? '📧' :
                                 app.status === 'approved' ? '⏳' :
                                 app.status === 'pending' ? '⏳' :
                                 '❌'}
                              </span>
                              {app.status === 'shipped' ? 'Onderweg' :
                               app.status === 'payment_selected' ? 'Klaar voor verzending' :
                               app.status === 'order_ready' ? 'Wacht op betaalmethode' :
                               app.status === 'approved' ? 'Wacht op bestelling binnen' :
                               app.status === 'pending' ? 'Wachtend op aanbetaling' :
                               'Afgewezen'}
                            </span>
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex flex-col space-y-1">
                            ...
                            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                              app.retailerApproved ? 'bg-green-100 text-green-800' :
                              app.retailerPending ? 'bg-yellow-100 text-yellow-800' :
                              app.retailerRejected ? 'bg-red-100 text-red-800' :
                              'bg-gray-100 text-gray-800'
                            }`}>
                              {app.retailerApproved ? '✅ Goedgekeurd' :
                               app.retailerPending ? '⏳ In behandeling' :
                               app.retailerRejected ? '❌ Afgewezen' :
                               '❓ Onbekend'}
                            </span>
                          </div>
                        </td>
                        <td className="px-6 py-4 text-right text-sm font-medium">
                          <div className="flex justify-end space-x-1 flex-wrap">
                            {/* Details Button */}
                              ...
                              className="inline-flex items-center justify-center w-28 px-2 py-1 border border-gray-300 text-xs font-medium rounded text-gray-700 bg-white hover:bg-gray-50 focus:outline-none mb-1"
                              >
                            ...
                            {app.canEdit ? (
                              <button
                                ...
                                className="inline-flex items-center justify-center w-28 px-2 py-1 border border-indigo-500 text-xs font-medium rounded text-white bg-indigo-500 hover:bg-indigo-600 focus:outline-none mb-1"
                              >
                              ...
                              <button
                                disabled
                                className="inline-flex items-center justify-center w-28 px-2 py-1 border border-gray-300 text-xs font-medium rounded text-gray-400 bg-gray-100 cursor-not-allowed mb-1"
                                title={
                                  app.retailerPending ? 'Retailer moet eerst goedgekeurd worden' :
                                  app.retailerRejected ? 'Retailer is afgewezen' :
                            ...
                            {app.retailerApproved && app.deposit_status === 'not_sent' && (
                              <button
                                onClick={() => handleSendDepositPayment(app.id)}
                                disabled={isProcessingPayment}
                                className="inline-flex items-center justify-center w-28 px-2 py-1 border border-green-500 text-xs font-medium rounded text-white bg-green-500 hover:bg-green-600 focus:outline-none disabled:opacity-50 mb-1"
                              >
                                💳 Aanbetaling
                              </button>
                            )}
                            ...
                            {app.deposit_status === 'paid' && !app.payment_options_sent && app.status === 'approved' && (
                              <button
                                onClick={() => handleSendOrderReady(app.id)}
                                disabled={isProcessingPayment}
                                className="inline-flex items-center justify-center w-32 px-2 py-1 border border-purple-500 text-xs font-medium rounded text-white bg-purple-500 hover:bg-purple-600 focus:outline-none disabled:opacity-50 mb-1"
                              >
                                📧 Bestelling Binnen
                              </button>
                            )}
                            ...
                            {(app.status === 'payment_selected' || (app.payment_method_selected && app.status !== 'shipped')) && (
                              <button
                                onClick={() => handleMarkAsShipped(app.id)}
                                disabled={isProcessingPayment}
                                className="inline-flex items-center justify-center w-28 px-2 py-1 border border-blue-500 text-xs font-medium rounded text-white bg-blue-500 hover:bg-blue-600 focus:outline-none disabled:opacity-50 mb-1"
                              >
                                📦 Verzenden
                              </button>
                            )}
                            ...
                            {app.status === 'shipped' && !app.product_delivered_at && (
                              <button
                                onClick={() => handleMarkAsDelivered(app.id)}
                                disabled={isProcessingPayment}
                                className="inline-flex items-center justify-center w-28 px-2 py-1 border border-green-500 text-xs font-medium rounded text-white bg-green-500 hover:bg-green-600 focus:outline-none disabled:opacity-50 mb-1"
                              >
                                ✅ Geleverd
                            ...
                            {app.product_delivered_at && app.remaining_payment_status === 'not_sent' && (
                              <button
                                onClick={() => handleSendRemainingPayment(app.id)}
                                disabled={isProcessingPayment}
                                className="inline-flex items-center justify-center w-28 px-2 py-1 border border-orange-500 text-xs font-medium rounded text-white bg-orange-500 hover:bg-orange-600 focus:outline-none disabled:opacity-50 mb-1"
                              >
                                💰 Restbedrag
                              </button>
                            )}
                            ...
                            <button
                              ...
                              className="inline-flex items-center justify-center w-28 px-2 py-1 border border-red-500 text-xs font-medium rounded text-white bg-red-500 hover:bg-red-600 focus:outline-none mb-1"
                              >
