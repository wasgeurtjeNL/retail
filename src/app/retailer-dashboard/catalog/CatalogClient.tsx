...
export default function CatalogPage() {
  ...
  const filteredProducts = products.filter(product => {
    ...
    const matchesSearch = !searchQuery || 
      ...
      product.description_frontend.toLowerCase().includes(searchQuery.toLowerCase());
  ...
  // Haal retailer email op uit auth context voor database operaties (regel 24)
  const handlePlaceOrder = async () => {
    ...
    try {
      console.log('[Frontend] Starting order creation for user:', user?.email);
      
      // Valideer user eerst (regel 24)
      if (!user?.email) {
        console.error('[Frontend] User email is missing:', user);
        alert('Er is een probleem met je account. Log opnieuw in en probeer het nog eens.');
        return;
      }
      
      // Bereid order data voor API call (regel 22 - uitgebreide metadata)
      const orderData = {
        ...
        shipping_address: '', // Wordt later ingevuld in address page
        shipping_city: '',
        shipping_postal_code: '',
        payment_method: 'invoice'
      };
      
      console.log('[Frontend] Sending order data to API:', orderData);
      
      // Verstuur naar database via API route (regel 18-25)
      ...
      console.log('[Frontend] Order created successfully:', result.order);
      
      // Maak localStorage compatible order voor backward compatibility
      ...
      // Save voor confirmation page
      localStorage.setItem('lastOrder', JSON.stringify(localStorageOrder));
      
      // Sla winkelwagen op voor recovery
      localStorage.setItem('retailerCart', JSON.stringify(cart));
      
      // Backward compatibility: sync met admin dashboard localStorage
      syncOrderToAdminDashboard(localStorageOrder);
      
      // Clear cart na succesvolle order
      clearCart();
      
      // Navigeer naar adreskeuze pagina
      router.push('/retailer-dashboard/orders/address');
      
    } catch (error) {
      console.error('[Frontend] Error creating order:', error);
      console.warn('[Frontend] Falling back to localStorage order creation');
      
      // Fallback: gebruik localStorage als API faalt (regel 25)
      const fallbackOrder = {
        id: `ORD-${Date.now()}`,
      ...
      // Sync naar admin dashboard
      syncOrderToAdminDashboard(fallbackOrder);
      
      // Clear cart
      clearCart();
      
      // Toon waarschuwing maar ga door
      alert('Let op: Je bestelling is lokaal opgeslagen vanwege een technisch probleem. Neem contact op met support als dit probleem zich herhaalt.');
      
      // Ga naar address page
      router.push('/retailer-dashboard/orders/address');
    } finally {
